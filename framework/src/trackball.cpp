...
Ray Trackball::generateRay(const glm::vec2& pixel, const Features& features) const
{
    ...
    if (features.extra.enableDepthOfField) {
        float lensRadius = features.apertureSize / 2.0f;
        glm::vec3 randomOff = lensRadius * offset();

        glm::vec3 C = ray.origin + features.focalLength * ray.direction;
        ray.origin = position() + randomOff;

        ray.direction = normalize(C - ray.origin);
    }
...
glm::vec3 offset()
{
    ...
    std::random_device randomDevTwo;
    std::mt19937 mtGenTwo(randomDevTwo());

    std::uniform_real_distribution<> generatorU(-1.0, 1.0);

    while (true) {
        auto p = glm::vec3 { generatorU(mtGen), generatorU(mtGenTwo), 0.0f };
        if (std::pow(glm::length(p), 2) >= 1) {
            continue;
        }
        return p;
    }
