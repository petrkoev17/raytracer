...
int main(int argc, char** argv)
{
    ...
    if (!config.cliRenderingEnabled) {
        ...
        bool debugSha = false;
        int factorBloomEffect = 0;
        bool debugBloomEffect = false;
            ...
            if (action == GLFW_PRESS) {
                ...
                case GLFW_KEY_R: {
                    ...
                    const auto tmp = window.getNormalizedCursorPos();
                        optDebugRay = camera.generateRay(tmp * 2.0f - 1.0f, features);
                } break;
                case GLFW_KEY_A: {
                    debugBVHLeafId++;
                } break;
        ...
        while (!window.shouldClose()) {
            ...
            if (ImGui::CollapsingHeader("Features", ImGuiTreeNodeFlags_DefaultOpen)) {
                ...
                ImGui::Checkbox("Show visited boxes", &config.features.showVisitedBoxes);
                ...
                ImGui::SliderInt("Number of samples", &config.features.amountOfSamples, 1, 100);
            }
            ImGui::Separator();
            ...
            if (ImGui::CollapsingHeader("Extra Features")) {
                ...
                if (&config.features.extra.enableBloomEffect) {
                    ImGui::SliderInt("Brightness factor", &factorBloomEffect, 1, 100);
                }
                ...
                if (config.features.extra.enableMultipleRaysPerPixel)
                    ImGui::SliderInt("Amount of rays per pixel", &config.features.amountOfRays, 1, 1000);
                ImGui::Checkbox("Transparency", &config.features.extra.enableTransparency);
                ImGui::Checkbox("Depth of field", &config.features.extra.enableDepthOfField);
                ImGui::SliderFloat("Focal length", &config.features.focalLength, 1.0f, 50.0f);
                ImGui::SliderFloat("Aperture size", &config.features.apertureSize, 0.0f, 10.0f);
            }
            ImGui::Separator();
            ...
            if (ImGui::Button("Render to file")) {
                ...
                if (result == NFD_OKAY) {
                    ...
                    if (config.features.extra.enableBloomEffect && debugBloomEffect) {
                        screen.bloomFilterDebug(factorBloomEffect);
                    } else if (config.features.extra.enableBloomEffect) {
                        screen.bloomFilter(factorBloomEffect);
                    }
            ...
            if (viewMode == ViewMode::Rasterization) {
                ...
                ImGui::Checkbox("Normal Interpolation", &config.features.debugFlags.debugNormalInterpolation);
                ImGui::Checkbox("Draw Sha split", &debugSha);
                if (debugSha)
                    ImGui::SliderInt("SHA internal", &internalIdx, 0, bvh.numOfInternalNodes()-1);
                ImGui::Checkbox("Debug bloom", &debugBloomEffect);
                ImGui::Checkbox("Debug glossy", &debugGlossyReflection);
            }
            ...
            if (x)
                ImGui::SliderFloat("x", &config.features.motion.x, -1, 1);
            ImGui::Checkbox("y-axis", &y);
            if (y)
                ImGui::SliderFloat("y", &config.features.motion.y, -1, 1);
            ImGui::Checkbox("z-axis", &z);
            if (z)
                ImGui::SliderFloat("z", &config.features.motion.z, -1, 1);
            ...
            case ViewMode::Rasterization: {
                ...
                if (debugGlossyReflection && optDebugRay) {
                    enableDebugDraw = true;
                    glDisable(GL_LIGHTING);
                    glDepthFunc(GL_LEQUAL);
                    (void)getFinalColor(scene, bvh, *optDebugRay, config.features);
                    (void)getGlossyReflection(*optDebugRay, bvh, scene, config.features);
                } else if (optDebugRay) {
                ...
                if (debugBVHLevel || debugBVHLeaf || debugSha) {
                    glPushAttrib(GL_ALL_ATTRIB_BITS);
                    setOpenGLMatrices(camera);
                    glDisable(GL_LIGHTING);
