...
bool checkIfValid(int width, int height, int currWidth, int currHeight) {
...
void Screen::bloomFilter(int factor) {
    std::vector<glm::vec3> bloom {};
    bloom.reserve(m_textureData.size());
    float treshold = factor / 100.0f;

    // Only keep large values
    for (int i = 0; i < m_textureData.size(); i++) {
        glm::vec3 currColor = m_textureData[i];
        float sum = 0.2126f * currColor.r + 0.7152f * currColor.g + 0.0722f * currColor.b;
        if (sum >= treshold) {
            bloom.push_back(currColor);
        } else {
            bloom.push_back(glm::vec3 { 0.0f });
        }
    }
    ...
    for (int i = 0; i < m_resolution.y; i++) {
        for (int j = 0; j < m_resolution.x; j++) {
            int current = indexAt(j, i);
            glm::vec3 sum { 0.0f };
            for (int a = -5; a < 6; a++) {
                for (int b = -5; b < 6; b++) {
                    if (checkIfValid(width, height, j + a, i + b)) {
                        sum += bloom[indexAt(j + a, i + b)]; 
                    }
                }
            }
            m_textureData[indexAt(j, i)] += (3 / 121.0f) * sum;
        }
    }
}

void Screen::bloomFilterDebug(int factor) {
    std::vector<glm::vec3> bloom {};
    bloom.reserve(m_textureData.size());
    float treshold = factor / 100.0f;
    ...
    for (int i = 0; i < m_textureData.size(); i++) {
        glm::vec3 currColor = m_textureData[i];
        float check = glm::max(currColor.r, glm::max(currColor.g, currColor.b));
        if (check >= treshold) {
            bloom.push_back(currColor);
        } else {
            bloom.push_back(glm::vec3 { 0.0f });
    ...
    for (int i = 0; i < m_resolution.y; i++) {
        for (int j = 0; j < m_resolution.x; j++) {
            ...
            m_textureData[indexAt(j, i)] = (1 / 121.0f) * sum;
        }
    }
}
