...
void drawNormals(const Vertex& v0, const Vertex& v1, const Vertex& v2, const glm::vec3& p, const glm::vec3 interpolatedNormal) {
    std::vector<Vertex> vertices { v0, v1, v2 };
    glBegin(GL_LINES);
    glColor3f(0.0f, 1.0f, 0.0f);
    for (int i = 0; i < 3; i++) {
        Vertex current = vertices[i];
        glVertex3fv(glm::value_ptr(current.position));
        glVertex3fv(glm::value_ptr(current.position + current.normal));
    }
    glEnd();
    glBegin(GL_LINES);
    glColor3f(1.0f, 0.0f, 0.0f);
    glVertex3fv(glm::value_ptr(p));
    glVertex3fv(glm::value_ptr(p + interpolatedNormal));
    glEnd();
